...
def augment(image, label):
    ...
    """
    # 1. Flips (TensorFlow handles batched 4D tensors automatically)
    image = tf.image.random_flip_left_right(image)
    image = tf.image.random_flip_up_down(image)

    # 2. Random Zoom using resize (batch-safe alternative to random_crop)
    original_shape = tf.shape(image)  # [batch_size, height, width, channels]
    ...
    # Resize to smaller size, then back to original (simulates zoom)
...
def create_data_loader(train_dir, test_dir, image_size=(224, 224), batch_size=64):
    # Load raw [0, 255] data
